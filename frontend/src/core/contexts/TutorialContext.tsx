...
export const TUTORIAL_STEPS: TutorialStep[] = [
  ...
  },
  {
    ...
    description: 'Último paso: revisá Configuración. En "Perfil" guardás tu especialidad. En "Preferencias" elegís modo oscuro o claro, configurás el recordatorio de notificaciones para tus cirugías, y si sos Premium podés apagar el toggle de "Publicidad" — separado de "Recordatorios y solicitudes" (cirugías, invitaciones, colegas), que siempre llega. Los avisos importantes del sistema (versión nueva, etc.) tampoco se pueden apagar, sin importar el plan. Si te queda alguna duda — sobre suscripción, referidos o privacidad — encontrás las Preguntas frecuentes en esta misma pestaña "Tutorial".',
    hint: 'El recordatorio de cirugía te avisa con anticipación — podés elegir cuántas horas antes querés recibirlo.',
  },
];

// Tutorial para anestesiólogos — es su propio recorrido, no una copia del de
// cirujano con nombres cambiados. Diferencias reales verificadas en el código:
// - /operations SÍ sirve para anestesiólogos: "Anestesiología" es una categoría
//   de primer nivel en operations.tsx (folderStructure), con las mismas 15
//   subcategorías (Cabeza, Cuello, Tórax...) que carga AnesthesiaCodesPicker
//   dentro del formulario del caso — es la misma Tabla de Códigos, filtrada
//   automáticamente por especialidad ("Según tu especialidad").
// - La calculadora rápida (/calculator) no carga CSVs de anestesia todavía —
//   se deja el paso igual que para cirujanos, se ajustará más adelante.
// - Los casos de anestesia no tienen subida de fotos propia, pero si el caso
//   es uno al que te invitaron, SÍ podés ver (no subir) las imágenes que
//   cargó el médico principal — se muestran igual para cualquiera con acceso
//   al caso (verificado en detail.tsx: el upload está restringido a isOwner,
//   pero el grid de imágenes se renderiza para cualquier viewer).
// - "Médico principal" y "ayudante" en un caso propio de anestesia son texto
//   libre, no invitaciones con aceptar/rechazar — eso solo aplica cuando un
//   cirujano te invita a SU caso (ver paso "Cuando te invitan a una cirugía").
export const ANESTHESIA_TUTORIAL_STEPS: TutorialStep[] = [
  {
    step: 1, route: '/operations', emoji: '🩺', target: 'procedures-search',
    title: 'Buscá tus procedimientos',
    description: 'Acá está la Tabla de Códigos completa, filtrada automáticamente según tu especialidad — así que ya vas a ver primero los códigos de anestesia. Antes de crear tu primer caso, guardá los que más usás como favoritos.',
    hint: 'Escribí el código o nombre → tocá ⭐ para guardarlo como favorito. Se van a precargar como "Procedimientos Frecuentes" en cada caso que crees.',
  },
  {
    step: 2, route: '/hospitals', emoji: '🏥', target: 'hospitals-list',
    title: 'Configurá tu hospital',
    description: 'Registrá los hospitales donde trabajás. Tenerlos guardados te permite llevar estadísticas por lugar y agiliza la carga al crear cada caso — se completa automáticamente.',
    hint: 'Encontrá tu hospital → tocá ⭐ para guardarlo como favorito. Aparecerá primero al crear tus casos.',
  },
  {
    step: 3, route: '/insurances', emoji: '🛡️', target: 'insurances-list',
    title: 'Seguros médicos',
    description: 'Guardá los seguros que manejás frecuentemente. Al crear un caso se cargan solos, sin necesidad de escribirlos cada vez.',
    hint: 'Tocá ⭐ en los seguros que más usás para guardarlos.',
  },
  {
    step: 4, route: '/favorites', emoji: '⭐', target: 'favorites-list',
    title: 'Tus favoritos',
    description: 'Todo lo que marcaste con ⭐ aparece acá: códigos de anestesia, hospitales y seguros. Cuando creés un caso nuevo, estos se pre-cargan para que sea más rápido.',
    hint: 'Si no ves algo, volvé a la sección correspondiente y marcalo con ⭐.',
  },
  {
    step: 5, route: '/colleagues', emoji: '👥', target: 'colleague-code',
    title: 'Colegas',
    description: 'Tu código de colega es único. Compartilo con otros médicos para conectarte — con cirujanos que puedan invitarte a sus casos como anestesiólogo, o con cualquier otro colega.',
    hint: 'Copiá tu código (aparece arriba de la pantalla) y enviáselo a un colega por WhatsApp o mensaje.',
  },
  {
    step: 6, route: '/calendar', emoji: '📅', target: 'calendar-connect',
    title: 'Google Calendar',
    description: 'Conectá tu cuenta de Google y cada caso que registrés (propio o aceptado) aparece automáticamente en tu agenda con fecha, hora y hospital.',
    hint: 'Tocá "Conectar Google Calendar" y autorizá el acceso. Solo se necesita hacer una vez.',
  },
  {
  ...
  },
  {
    ...
    description: 'Seleccioná el hospital (tus favoritos aparecen primero) y el seguro — si no tiene, dejalo en "Sin seguro". Si conocés al médico principal y al ayudante de este caso, escribí sus nombres: son campos de texto libre, no hace falta que tengan cuenta en MeDico App ni que acepten nada.',
    hint: 'Médico Principal y Médico Ayudante son solo para referencia — a diferencia de cuando a VOS te invitan, acá no hay invitación ni aceptar/rechazar.',
  },
  {
    step: 11, route: '/cases/new/anesthesia', emoji: '🩺', target: 'anesthesia-codes-picker',
    title: '③ Códigos de anestesia',
    description: 'Buscá y agregá los códigos de anestesia que usaste — los mismos que viste en la Tabla de Códigos, ahora directo en el formulario. Tocá la ⭐ junto a un resultado para guardarlo como favorito — la próxima vez aparece arriba en "Procedimientos Frecuentes" para agregarlo con un solo toque.',
  ...
  },
  {
    ...
    description: 'Tocá "Ver" en cualquier caso para abrir el detalle completo. Desde ahí podés seguir agregando códigos y exportar el PDF de anestesia.',
